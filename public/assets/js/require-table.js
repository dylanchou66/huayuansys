define(['jquery', 'bootstrap', 'moment', 'moment/locale/zh-cn', 'bootstrap-table', 'bootstrap-table-lang', 'bootstrap-table-export', 'bootstrap-table-commonsearch', 'bootstrap-table-template', 'bootstrap-table-jumpto', 'bootstrap-table-fixed-columns'], function ($, undefined, Moment) {
    var Table = {
        ...
        config: {
            ...
            addbtn: '.btn-add',
            addprivate: '.btn-addprivate',
            addhetong: '.btn-addhetong',
            distribute:'.btn-distribute',
            recharge:'.btn-recharge',
            addre: '.btn-re',
            editbtn: '.btn-edit',
            delbtn: '.btn-del',
        ...
        api: {
            ...
            bindevent: function (table) {
                ...
                toolbar.on('click', Table.config.addprivate, function () {
                    var ids = Table.api.selectedids(table);
                    var url = options.extend.addprivate_url;
                    if (url.indexOf("{ids}") !== -1) {
                        url = Table.api.replaceurl(url, {ids: ids.length > 0 ? ids.join(",") : 0}, table);
                    }
                    Fast.api.open(url, $(this).data("original-title") || $(this).attr("title") || __('Add'), $(this).data() || {});
                ...
                toolbar.on('click', Table.config.addhetong, function () {
                    var ids = Table.api.selectedids(table);
                    var url = options.extend.addhetong_url;
                    if (url.indexOf("{ids}") !== -1) {
                        url = Table.api.replaceurl(url, {ids: ids.length > 0 ? ids.join(",") : 0}, table);
                    }
                    Fast.api.open(url, $(this).data("original-title") || $(this).attr("title") || __('Add'), $(this).data() || {});
                ...
                toolbar.on('click', Table.config.distribute, function () {
                    var ids = Table.api.selectedids(table);
                    var url = options.extend.distribute_url;
                    if (url.indexOf("{ids}") !== -1) {
                        url = Table.api.replaceurl(url, {ids: ids.length > 0 ? ids.join(",") : 0}, table);
                    }
                    Fast.api.open(url, $(this).data("original-title") || $(this).attr("title") || __('Add'), $(this).data() || {});
                ...
                toolbar.on('click', Table.config.recharge, function () {
                    var ids = Table.api.selectedids(table);
                    var url = options.extend.recharge_url;
                    if (url.indexOf("{ids}") !== -1) {
                        url = Table.api.replaceurl(url, {ids: ids.length > 0 ? ids.join(",") : 0}, table);
                    }
                    Fast.api.open(url, $(this).data("original-title") || $(this).attr("title") || __('Add'), $(this).data() || {});
                ...
                toolbar.on('click', Table.config.addre, function () {
                    var ids = Table.api.selectedids(table);
                    var url = options.extend.re_url;
                    if (url.indexOf("{ids}") !== -1) {
                        url = Table.api.replaceurl(url, {ids: ids.length > 0 ? ids.join(",") : 0}, table);
                    }
                    Fast.api.open(url, $(this).data("original-title") || $(this).attr("title") || __('Add'), $(this).data() || {});
