 ...
 * @icon fa fa-circle-o
 */
class CollectionInfo extends Backend
{
    
    /**
     * DeductionInfo模型对象
     * @var \app\admin\model\Collection
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\Collection;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            foreach ($list as $row) {
                ...
                switch($row['type']){
                    case 2:
                        $row->getRelation('project')->visible(['id','company']);
                        break;
                    case 1:
                        $row->getRelation('fgs')->visible(['id','name']);
                        break;
                }
            }
...
//                $nid = $T->where('orders_id',$id)->field('id')->find();
                ...
                if($new == 'pay_status'){
                    ...
                    if($check == 1 ){
                        ...
                        foreach ($check_ids as $v){
                            $Pay->where('id',$v)->update(array('collection'=>2,'collection_time'=>time()));
                        }
                        //判断是否完成付款，结果告知最终用户
                        ...
                        $check_ids = explode(',',$check_ids['ids']);
                        foreach ($check_ids as $v){
                            $Pay->where('id',$v)->update(array('collection'=>0));
                        }
                    }
                }
        ...
        $info = '';
        foreach($n_ids as $v){
            $r = $Pay->where(['id'=>$v])->field('pay_one_id')->find()->toArray();
            $info .= $r['pay_one_id'].',';
        }
        switch($row['type']){
            case 2:
                $P = new \app\admin\model\Project();
                $res = $P->where('id',$row['type_id'])->field('company')->find()->toArray();
                $row['cys_name'] = $res['company'];
                break;
            case 1:
                $D = new \app\admin\model\DictValues();
                //查询名称
                $res = $D->where('id',$row['type_id'])->field('name')->find()->toArray();
